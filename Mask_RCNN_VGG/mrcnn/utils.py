...
#    mask_1 = np.where(mask >= threshold, 1, 0).astype(np.int)
    # Put the mask in the right location.
    full_mask = np.zeros(image_shape[:2], dtype=np.bool)
    full_mask[y1:y2, x1:x2] = mask


#    with open('/home/jhwang/Mask_RCNN/samples/phorcom/log.txt','a') as file_object:
#        for i in range (len(mask_1)):
#            file_object.write(str(mask_1[i]) + '\n')
